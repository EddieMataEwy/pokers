...
impl HandRange {
    ...
    pub fn from_strings(arr: Vec<String>) -> Vec<Self> {
        let ranges: Vec<HandRange> = arr.iter()
    ...
    pub fn remove_conflicting_combos(&mut self, board_mask: u64, dead_mask: u64) {
        self.hands
            .retain(|x| (((1u64 << x.0) | (1u64 << x.1)) & (board_mask | dead_mask)) == 0);
    }
    ...
    fn parse_hand(&mut self, i: &mut usize) -> bool {
        ...
        let mut r3: u8 = u8::MAX;
        let mut r4: u8 = u8::MAX;
        ...
        if explicit_suits {
            ...
            if self.parse_char(i, '+') {
            ...
            } else if self.parse_char(i, '-'){
                if !self.parse_combos_dash(r1, r2, &mut r3, &mut r4, &mut suited, &mut offsuited, i) {
                    *i = backtrack;
                    return false;
                }
                if self.parse_char(i, '@') {
                ...
                self.add_combos_dash(r1, r2, r3, r4, suited, offsuited, weight);
            } else {
                if self.parse_char(i, '@') {
                    self.parse_weight(i, &mut weight);
                }
    ...
    fn parse_combos_dash(&mut self, rank1: u8, rank2: u8, rank3: &mut u8, rank4: &mut u8, suited: &mut bool, offsuited: &mut bool, i: &mut usize) -> bool {
        if !self.parse_rank(i, rank3){
            return false;
        }
        if !self.parse_rank(i, rank4) {
            return false;
        }
        if self.parse_char(i, 'o'){
            if *offsuited != true {return false;}
            if *suited {return false;}
        }
        if self.parse_char(i, 's'){
            if *suited != true {return false;}
            if *offsuited {return false;}
        }
        ...
        if &rank1 != rank3 && &rank1 != rank4 && &rank2 != rank3 && &rank2 != rank4 {
            return false;
        }
        true
    }
    ...
    fn add_combos_dash(&mut self, rank1: u8, rank2: u8, rank3: u8, rank4: u8, suited: bool, offsuited: bool, weight: u8){
        if rank1 == rank2 {
            for r in rank3..=rank1{
                self.add_combos(r, r, suited, offsuited, weight);
            }
        } else {
            if rank1 == rank3 {
                if rank2 > rank4 {
                    for r in rank4..=rank2{
                        self.add_combos(rank1, r, suited, offsuited, weight);
                    }
                } else {
                    for r in rank2..=rank4{
                        self.add_combos(rank1, r, suited, offsuited, weight);
                    }
                }
            } else if rank1 == rank4 {
                if rank2 > rank3 {
                    for r in rank3..=rank2{
                        self.add_combos(rank1, r, suited, offsuited, weight);
                    }
                } else {
                    for r in rank2..=rank3{
                        self.add_combos(rank1, r, suited, offsuited, weight);
                    }
                }
            } else if rank2 == rank3 {
                if rank1 > rank4 {
                    for r in rank4..=rank1{
                        self.add_combos(rank2, r, suited, offsuited, weight);
                    }
                } else {
                    for r in rank1..=rank4{
                        self.add_combos(rank2, r, suited, offsuited, weight);
                    }
                }
            } else if rank2 == rank4 {
...
pub fn card_to_string(card: u8) -> String {
    String::from_iter([
        RANK_TO_CHAR[usize::from(card >> 2)],
        SUIT_TO_CHAR[usize::from(card & 3)]
    ])
}
